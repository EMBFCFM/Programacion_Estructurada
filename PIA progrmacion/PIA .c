...
char** split_str(char* str, const char a_delim) {
	...
	delim[0] = a_delim;
	delim[1] = '\0';
	//este while cuenta cuantos elementos seran extraidos
	while(*str_tmp) {
		if(*str_tmp == a_delim) {
			count++;
			last_delim = str_tmp;
	...
	//esta variable agregara espacio para el token final
	count += last_delim < (str + strlen(str) - 1);
	//esta variable agregara espacio para el caracter de final de cadena
	count++;
...
int main() {
	...
	saveData_at_file("salidaCal.csv",alumn,calificacionesP,g,TAM,7);
